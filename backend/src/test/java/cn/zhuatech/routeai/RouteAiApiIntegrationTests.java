 ...
 */
@SpringBootTest @AutoConfigureMockMvc class RouteAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanPlanRoute()throws Exception{mvc.perform(post("/api/ai/route/plan").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"routeNo\":\"RT-88\",\"distanceKm\":32,\"stopCount\":4,\"trafficIndex\":0.20,\"weatherRisk\":0.10,\"vehicleLoadPercent\":72,\"driverHoursToday\":3,\"coldChain\":false,\"maximumColdChainMinutes\":300}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.dispatchDecision").value("DISPATCH"));}}
